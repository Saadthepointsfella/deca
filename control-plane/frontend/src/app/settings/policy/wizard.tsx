                ...
                <div className="flex flex-col gap-1 text-xs">
                  <button
                    className={`px-2 py-1 rounded text-left ${
                      posture[tier] === "STRICT" ? "bg-white text-black" : "border border-ink-700"
                    }`}
                  ...
                  <button
                    className={`px-2 py-1 rounded text-left ${
                      posture[tier] === "BALANCED" ? "bg-white text-black" : "border border-ink-700"
                    }`}
                  ...
                  <button
                    className={`px-2 py-1 rounded text-left ${
                      posture[tier] === "GENEROUS" ? "bg-white text-black" : "border border-ink-700"
                    }`}
                    onClick={() => setPosture({ ...posture, [tier]: "GENEROUS" })}
                  >
                    Generous
                    <span className="block text-ink-600">More cushion and spike tolerance.</span>
                  </button>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div>
              ...
              <div className="flex flex-col gap-1">
                <button
                  className={`px-2 py-1 rounded text-left ${
                    spikeTolerance === "LOW" ? "bg-white text-black" : "border border-ink-700"
                  }`}
                ...
                <button
                  className={`px-2 py-1 rounded text-left ${
                    spikeTolerance === "MEDIUM" ? "bg-white text-black" : "border border-ink-700"
                  }`}
                ...
                <button
                  className={`px-2 py-1 rounded text-left ${
                    spikeTolerance === "HIGH" ? "bg-white text-black" : "border border-ink-700"
                  }`}
                  onClick={() => setSpikeTolerance("HIGH")}
                >
                  High
                  <span className="block text-ink-600">Let spikes through unless extreme.</span>
                </button>
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-col gap-1">
                <button
                  className={`px-2 py-1 rounded text-left ${
                    freeExperience === "MINIMAL" ? "bg-white text-black" : "border border-ink-700"
                  }`}
                  onClick={() => setFreeExperience("MINIMAL")}
                >
                  Minimal
                  <span className="block text-ink-600">Free can be starved under load.</span>
                </button>
                <button
                  className={`px-2 py-1 rounded text-left ${
                    freeExperience === "STANDARD" ? "bg-white text-black" : "border border-ink-700"
                  }`}
                  onClick={() => setFreeExperience("STANDARD")}
                >
                  Standard
                  <span className="block text-ink-600">Some reserved capacity for Free.</span>
                </button>
                <button
                  className={`px-2 py-1 rounded text-left ${
                    freeExperience === "STRONG" ? "bg-white text-black" : "border border-ink-700"
                  }`}
                  onClick={() => setFreeExperience("STRONG")}
                >
                  Strong
                  <span className="block text-ink-600">Free can’t be starved, ever.</span>
                </button>
              </div>
            </div>
          </div>
      ...
      <Card>
        ...
        <CardBody>
          <p className="text-xs text-ink-400 mb-2">
            This will overwrite the current policy with a configuration derived from your choices.
          </p>
          {preview && (
          ...
          {!preview && (
            <p className="text-xs text-ink-500">
              After you apply once, a human-readable summary of the resulting policy will appear here.
            </p>
          )}
          {!canAdmin && (
...
function buildConfigFromWizard(
  ...
  for (const tier of TIERS) {
    ...
    if (p === "STRICT") {
      ...
      out.usage.throttle.max_delay_ms[tier] = BASE_TEMPLATE.usage.throttle.max_delay_ms[tier];
    } else {
      // GENEROUS
      out.usage.soft_multiplier[tier] = 1.1;
      out.usage.hard_multiplier[tier] = 1.8;
      out.usage.throttle.max_delay_ms[tier] = tier === "ENTERPRISE" ? 400 : 600;
    }
  }
...
function summaryFromConfig(cfg: PolicyConfigV2): string {
  ...
  lines.push(
    `  ENT:  throttle>${cfg.spikes.throttle_above.ENTERPRISE}, block>${cfg.spikes.block_above.ENTERPRISE ?? "none"}`
  );
  lines.push("");
  lines.push(
